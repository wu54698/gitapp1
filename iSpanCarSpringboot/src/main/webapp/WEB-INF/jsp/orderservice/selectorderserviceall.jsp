					...
					<div id="content">
						...
						<div class="container-fluid">
							...
							<h1>預約資訊</h1>
								...
								<form method="POST">
						<label>查詢預約姓名：</label><input type="text" name="ordername"
							required /> <input type="submit" value="提交"
							formaction="/backstage/orderserviceSelect.controller"> </form>
						 <br> <br> <br>
							...
							<table border="1">
								...
								<c:forEach var="orderservice" items="${orderSelectall}"
									varStatus="vs">
									...
									<tr>
										...
										<td class="ordername">${orderservice.ordername}</td>
										<td>${orderservice.orderemail}</td>
										<td>${orderservice.orderphonenumber}</td>
										<td>${orderservice.orderdate}</td>
										<td>${orderservice.ordertime}</td>
										<td>${orderservice.orderproduct}</td>
										<td>${orderservice.orderremarks}</td>
										<td><form action="JumptoUpdateOrderService" method="post">
												<input type="hidden" value="${orderservice.ordername}"
													name="ordername">
							...
							<br>
							<input type="button" value="新增預約"
								onclick="location.href='/service.signinorderService'">							
							
								 <input
								type="button" onclick="location.href='serviceAllController'"
								value="回到保養廠"></input> 
								
								 <input
								type="button" onclick="location.href='serviceexportCsv.controller'"
								value="csv"></input>
								
								 <input
								type="button" onclick="location.href='serviceexportJson.controller'"
								value="json"></input>	
			...
			$('#content').on('click','.delete', function () {
			     ...
			     Swal.fire({
			        ...
			        if (result.isConfirmed) {
			          ...
			          console.log($(this).parent().parent().children('.ordername').text()+'this');
			         
			     $.ajax({
			      type: "POST",
			      url: "<c:url value='/backstage/orderserviceDeleteController'/>",
			      //contentType text
			      context: this,
			      dataType: "text",
			      data: {
			       "ordername": $(this).parent().parent().children(
			        '.ordername').text()
			      },
