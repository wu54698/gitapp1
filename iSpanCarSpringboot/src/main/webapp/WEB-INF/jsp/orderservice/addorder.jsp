...
<body id="page-top">
		...
		<div id="content-wrapper" class="d-flex flex-column">
			...
			<div id="content">
				...
				<div class="container-fluid"
					...
					<!-- Page Heading -->
					<h1 class="h3 mb-4 text-gray-800">新增預約</h1>
					<form id="signin" action="orderserviceInsert.controller" method="POST"
						enctype="multipart/form-data">
					<label>保養廠名稱：</label><input type="text" name="servicename" id ="servicename"required /><br>
					<label>預約姓名：</label><input type="text" name="ordername" id="ordername" required /><br>
					<label>預約電子郵件：</label><input type="text" name="orderemail" id="orderemail" required /><br>
					<label>預約電話：</label><input type="text" name="orderphonenumber" id="orderphonenumber"><br>
					<label>預約日期：</label> <input type="date" name="orderdate" id="orderdate"
						min="<%=new java.text.SimpleDateFormat("yyyy-MM-dd").format(new java.util.Date())%>">
					<br> <label for="ordertime" class="form-label">預約時間：</label> <input type="time" id="ordertime" name="ordertime">
							<br> <label>預約項目：</label> <select name="orderproduct" id="orderproduct">
					...
					</select><br> <label>備註：</label>
					<textarea name="orderremarks" id="orderremarks" rows="6" cols="40" required></textarea><br>
					<input type="submit" value="新增" 
					formaction="backstage/orderserviceInsert.controller">
						<input type="button" value="取消"
							onclick="location.href='/backstage/orderserviceAllController'">
	...
	$('#inputForm').on('click', function(){
		$('#servicename').val('資展國際車業')
		$('#ordername').val('余先生')
		$('#orderemail').val('a0926855118@gmail.com')
		$('#orderphonenumber').val('0926855118')
		$('#orderdate').val('2023-02-22')
		$('#ordertime').val('15:30')
		$('#orderproduct').val('保養相關')
		$('#orderremarks').val('小保養換機油就好 車型是golf　謝謝你們')
